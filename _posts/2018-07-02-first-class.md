...
**분류 모형의 평가**
1. 교차표
  - 정분류율, 오분류율, 정확도(민감도), 특이도, 재현도, 소수 집단 정분류율, 소수 집단 오분류율, F 
2.  ROC 곡선
3. 이익도표
4. 리프트 도표
...
#### 정리
방향성 데이터 마이닝 -> '모형 생성 단계', '스코어링 단계' 로 구분되고 단계에 따라 활용되는 데이터 셋은 훈련용, 검증용, 테스트용으로 구분된다.
균형화 -> 소수 집단과 다수 집단간의 균형화를 위한 "과대 샘플링", "과소 샘플링"
과적합 -> **훈련용 데이터 집합만이 가지는 특수한 패턴**까지도 함께 학습하는 현상을 과적합이라 함. 표본에 특화되어 Noise를 test data까지 끌고옴.
모형의 평가 -> 분류 문제: 교차표, ROC그래프, 이익도표, 리프트 도표. 추정문제: 실제값과 예측값의 차이(**잔차**)를 활용
